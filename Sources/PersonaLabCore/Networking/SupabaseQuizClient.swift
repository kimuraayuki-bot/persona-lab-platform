...
public final class SupabaseQuizClient: QuizDataClientProtocol {
    ...
    public func createQuiz(quiz: Quiz, creatorID: UUID, accessToken: String) async throws -> Quiz {
        ...
        for question in quiz.questions.sorted(by: { $0.order < $1.order }) {
            let questionRows: [QuestionRow] = try await send(
                path: "questions",
                method: "POST",
                queryItems: nil,
                body: [CreateQuestionRow(quizID: createdQuiz.id, prompt: question.prompt, orderIndex: question.order)],
                accessToken: accessToken,
                preferRepresentation: true
            )
        ...
        return Quiz(
            ...
            questions: createdQuestions.sorted(by: { $0.order < $1.order }),
